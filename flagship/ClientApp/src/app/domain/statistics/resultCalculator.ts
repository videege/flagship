...
export class ResultCalculator {
    ...
    public probabilityOfSuccess(probabilities: number[], successes: number): number {
        ...
        if (successes == 0) {
            this.computedProbabilities[0] = probabilities.reduce((product, current) => product * (1 - current));
        } else if (successes > probabilities.length) {
            return 0; // impossible to get more than N successes out of N rolls
        } else {
            let probability: number = 0;
            for (let i = 1; i <= successes; i++) {
                let ratios = probabilities.map(p => Math.pow(p / (1 - p), i))
